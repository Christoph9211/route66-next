export function wcagTextColor(bg: string) {
...
export function applyAutoContrast(root = document) {
  ...
  root.querySelectorAll('.auto-contrast').forEach((el: Element) => {
    (el as HTMLElement).style.color = wcagTextColor(getEffectiveBG(el));
  });
}
